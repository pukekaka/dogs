...
babl_path = os.path.join(current_directory, data_directory_name)
...
def default_write(f, value):
        f.write(str(value) + "\t")

def main():
    # args = get_args_parser().parse_args()
    preprocess = Preprocess(babl_path)
    preprocess.set_path()
    ...
    with open(os.path.join('config.txt'), 'w') as f:
        ...
        default_write(f, batch_size)
        default_write(f, hidden_units)
        default_write(f, learning_rate)
        default_write(f, iter_time)
        default_write(f, display_step)
